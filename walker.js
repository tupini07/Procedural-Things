...
walker.setDir = function(nDir){
  walker.dir = nDir > 360 ? walker.setDir(nDir - 360) : nDir;
};
...
walker.walk = function(distance){
  var magX = distance * Math.cos(walker.dir); //the magnitude to move in the X direction
  var magY = distance * Math.sin(walker.dir); //the magnitude to move in the Y direction
  walker.x += magX;
  walker.y += magY;
...
walker.move = function(newX, newY){
  walker.x = newX;
  walker.y = newY;
